...
2. Create or update a Slack app from `manifest.json`, then enable the **Agents** feature in the app's Slack settings. Agent View is required for the Messages tab and suggested prompts.
...
## Security
Secrets are redacted from logs and Slack-facing errors. Only `http:` and `https:` evidence links are rendered. Production-like environments require a short-lived, user-bound confirmation card before Talos starts the run.
...
## Known limitations
Real-Time Search is feature-flagged and not implemented in the core flow. Production confirmation uses a Block Kit card rather than a modal.
